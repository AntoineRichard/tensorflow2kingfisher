...
class DreamerAgent:
    def __init__(self):
        # DREAMER
        self.agent = None
        self.save_directory = '/mnt/nvme-storage/antoine/DREAMER/dreamer/logdir/kf_sim/dreamer/20_float32/'
        self.episode = {}
	...
	self.precision = 16
        ...
        self.obs['image'] = np.zeros((1,64,64,3),dtype=np.float32)
        self.obs['reward'] = np.zeros((1))
        # ROS
        self.drive = Drive()
        rospy.Subscriber('/cmd_drive', Drive, self.CMDCallback, queue_size=1)
    ...
    def laserCallback(self, obs):
        if self.skip:
            ...
            self.skip = True
            ranges = np.min(np.reshape(np.nan_to_num(np.array(obs.ranges)),[-1,2]),axis=1)
            self.laser = np.expand_dims(ranges * (ranges < 30),axis=-1)#np.reshape(np.fromstring(obs.data, np.uint8),[64,64,3])
            if not self.Done:
                self.obs['laser'][0] = self.laser
                self.obs['reward'][0] = self.reward
                self.incrementEpisode(self.laser, self.actions, self.obs['reward'][0])
                self.step += 1
    ...
    def cmdCallback(self,msg):
        self.actions = np.array(msg.left, msg.right)
