...
class ConvEncoder2(tools.Module):
  ...
  def __call__(self, obs):
    kwargs = dict(strides=2, activation=self._act)
    kwargs2 = dict(strides=1, activation=self._act)
    x = tf.reshape(obs['image'], (-1,) + tuple(obs['image'].shape[-3:]))
    x = self.get('h1', tfkl.Conv2D, 1 * self._depth, 4, **kwargs)(x)
    x = self.get('h2', tfkl.Conv2D, 2 * self._depth, 4, **kwargs)(x)
    x = self.get('h3', tfkl.Conv2D, 4 * self._depth, 4, **kwargs)(x)
    x = self.get('h4', tfkl.Conv2D, 8 * self._depth, 4, **kwargs)(x)
    x = self.get('h5', tfkl.Conv2D, 8 * self._depth, 2, **kwargs)(x)
    shape = tf.concat([tf.shape(obs['image'])[:-3], [8 * self._depth]], 0)
    return tf.reshape(x, shape)
...
class ConvDecoder2(tools.Module):
