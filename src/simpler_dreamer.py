...
class Dreamer(tools.Module):
  ...
  @tf.function
  def test(self, obs, state, training):
    if state is None:
      latent = self._dynamics.initial(len(obs['image']))
      action = tf.zeros((len(obs['image']), self._actdim), self._float)
    else:
    ...
    obs = obs.copy()
    obs['image'] = tf.cast(obs['image'][0],dtype)
    #with tf.device('cpu:0'):
  ...
  #@tf.function
  #def cast_image(self,obs):
  #    dtype = prec.global_policy().compute_dtype
  #    obs['image'] = tf.cast(obs['image'][0],dtype)

  def load(self, filename):
    super(Dreamer,self).load(filename)
...
#  try:
#    config.logdir.mkdir(parents=True)
#  except:
#    pass
...
#  #print(random_agent(0, range(1), 0))
#  #print(random_agent(0, range(2), 0))
#  #print(random_agent(1, range(2), 2))
#  #print(random_agent(1, range(3), 2))
...
#  agent = Dreamer(config, datadir, actspace)
#
#  if (config.logdir / 'variables.pkl').exists():
#    print('Load checkpoint.')
#    agent.load(config.logdir / 'variables.pkl')
#  #state = None
#  #print('Trying to run train step')
#  #agent.train(next(agent._dataset), False)
#  #print('Train step OK')
#  #agent.train(next(agent._dataset), False)
#  #print('Train step OK')
#  #agent.train(next(agent._dataset), False)
...
#  training=False
#  episode = np.load('/mnt/nvme-storage/antoine/DREAMER/dreamer/logdir/turtle_sim/dreamer/1/episodes/20200512T220956-50b634404bcb477fa1d2520e0ec80ce7-502.npz')
#  episode = {k: episode[k] for k in episode.keys()}
#  state=None
#  keys = list(episode.keys())#['orientations','height','velocity','image','reward']
